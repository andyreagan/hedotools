...
return s}!function(){d3.urllib={encoder:function(){function t(t){return{current:n}}function e(){GET={},query=window.location.search.substring(1).split("&")
...
var hedotools=hedotools||{},classColor=d3.scale.quantize().range([0,1,2,3,4,5,6]).domain([50,1])
...
hedotools.shifter.setfigure(d3.select("#shift01")).setText(["Why "+allData[shiftComp].name+" is "+o+" than "+allData[shiftRef].name+":"]).plot()}}}}(),hedotools.lens=function(){var t,e,r,n,a,o=d3.urllib.encoder().varname("lens"),s=d3.urllib.decoder().varname("lens").varresult([4,6]),i={top:0,right:55,bottom:0,left:0},l=100-i.top-i.bottom,c=.875*l-5,u=function(){r=parseInt(t.style("width"))-i.left-i.right,n=.875*r-5,a=.125*r},f=function(e){return console.log("setting figure"),t=e,u(),hedotools.lens},h=function(t){return e=t,hedotools.lens}
lensExtent=s().cached
...
a=n(l,i,t+1/3),o=n(l,i,t),s=n(l,i,t-1/3)}return[Math.round(255*a),Math.round(255*o),Math.round(255*s)]}var e,r,n=function(t){return console.log("setting figure"),e=t,hedotools.map}
return{setfigure:n,setdata:function(t){return r=t,hedotools.map},plot:function(){function n(t,e){activeHover?(activeHover=!1,d3.selectAll(".state").style("fill",null),stateSelType,d3.selectAll(".state."+allData[e].name[0]+allData[e].name.split(" ")[allData[e].name.split(" ").length-1]).attr("stroke-width",3)):(activeHover=!0,d3.selectAll(".state").attr("stroke-width",.7))}function a(t,e){var r=this.getBBox(),n=Math.floor(r.x+r.width/2),a=Math.floor(r.y+r.height/2),o="Words Used: "+commaSeparateNumber(d3.sum(allData[e].freq)),s="Total Words: "+commaSeparateNumber(d3.sum(allData[e].rawFreq)),l="US Words Used: "+commaSeparateNumber(d3.sum(allData[51].freq)),u="US Total Words: "+commaSeparateNumber(d3.sum(allData[51].rawFreq)),f="Average Happiness: "+allData[e].avhapps.toFixed(2)
hoverboxheight=176,hoverboxwidth=d3.max([o.width("13px arial"),f.width("15px arial"),s.width("13px arial"),l.width("13px arial"),u.width("13px arial")])+20,hoverboxxoffset=60
var h=30
n+hoverboxwidth+hoverboxxoffset>i&&(hoverboxxoffset=-hoverboxxoffset-hoverboxwidth),a-hoverboxheight/2-h<0&&(h=-30)
var d=c.append("g").attr({class:"hoverinfogroup",transform:"translate("+(n+hoverboxxoffset)+","+(a-hoverboxheight/2-h)+")"})
if(d.append("rect").attr({class:"hoverinfobox",x:0,y:0,width:hoverboxwidth,height:hoverboxheight,fill:"white",stroke:"black"}),d.append("text").attr({class:"hoverinfotext",x:10,y:15,"font-size":15}).text(allData[e].name),d.append("line").attr({class:"hoverinfotext",x:10,y:15,"font-size":15}).text(allData[e].name),d.append("text").attr({class:"hoverinfotext",x:10,y:38,"font-size":17}).text("Rank:"),d.append("text").attr({class:"hoverinfotext",x:59,y:55,"font-size":40}).text(sortedStateList[e]),d.append("text").attr({class:"hoverinfotext",x:105,y:56,"font-size":20}).text("out of 51"),d.append("text").attr({class:"hoverinfotext",x:10,y:79,"font-size":15}).text(f),d.append("text").attr({class:"hoverinfotext",x:10,y:97,"font-size":13}).text(o),d.append("text").attr({class:"hoverinfotext",x:10,y:114,"font-size":13}).text(s),d.append("text").attr({class:"hoverinfotext",x:10,y:131,"font-size":13}).text("US Average Happiness: "+allData[51].avhapps.toFixed(2)),d.append("text").attr({class:"hoverinfotext",x:10,y:148,"font-size":13}).text(l),d.append("text").attr({class:"hoverinfotext",x:10,y:165,"font-size":13}).text(u),activeHover&&(stateSelType?(shiftComp=e,d3.select(".complabel").text(allData[e].name),compencoder.varval(allData[e].name)):(shiftRef=e,d3.select(".reflabel").text(allData[e].name),refencoder.varval(allData[e].name)),d3.selectAll(".state."+allData[e].name[0]+allData[e].name.split(" ")[allData[e].name.split(" ").length-1]).style("fill","#428bca"),shiftRef!==shiftComp)){hedotools.shifter.shift(allData[shiftRef].freq,allData[shiftComp].freq,lens,words)
var p=hedotools.shifter._compH()>hedotools.shifter._refH()?"happier":"less happy"
hedotools.shifter.setfigure(d3.select("#shift01")).setText(["Why "+allData[shiftComp].name+" is "+p+" than "+allData[shiftRef].name+":"]).plot()}}function o(t,e){d3.select(".hoverinfogroup").remove(),activeHover&&d3.select(this).style("fill",null)}function s(){i=parseInt(e.style("width")),l=650*i/900,x.translate([i/2,l/2]).scale(1.3*i),c.selectAll("path").attr("d",m),c.attr("width",i).attr("height",l)}var i=parseInt(e.style("width")),l=650*i/900
e.select(".canvas").remove()
var c=e.append("svg").attr("class","map canvas").attr("id","mapsvg").attr("width",i).attr("height",l),u=[!1,!0],f=["Reference","Comparison"],h=f.map(function(t){return t.width()}),d=(u.length,d3.sum(h),d3.scale.linear().domain([340,730]).range([0,1])),p=d(i)
!function(t,e,r){var n=[0,1,2,3,4,5,6],a=[t,t,t,t,t,t,t],o=.25*n.length*2-.25+0+d3.sum(a),s=c.append("g").attr({class:"legendgroup",transform:"translate("+(i-50-o)+","+(l-e-e-2)+")"})
s.selectAll("rect.legendrect").data(n).enter().append("rect").attr({class:function(t,e){return"q"+e+"-8"},x:function(t,e){return 0===e?0:d3.sum(a.slice(0,e))+.25*e+.25*(e-1)+0},y:0,width:function(t,e){return a[e]},height:e,"stroke-width":"1",stroke:"rgb(0,0,0)"}),s.selectAll("text.legendtext").data(["less happy","happier"]).enter().append("text").attr({x:function(t,e){return 0==e?0:o-t.width(r+"px arial")},y:e+e,class:function(t,e){return"legendtext"},"font-size":r+"px"}).text(function(t,e){return t})}(20+10*p,8+5*p,9+3*p)
var x=d3.geo.albersUsa().translate([i/2,l/2]).scale(1.3*i),m=d3.geo.path().projection(x),g=[240,60],v=Array(20),y=Array(20)
for(w=0;w<20;w++)v[w]=t((g[0]+(g[1]-g[0])/19*w)/360,1,.5),y[w]="rgb("+v[w][0]+","+v[w][1]+","+v[w][2]+")"
color=d3.scale.quantize().range(y).domain([d3.min(allData,function(t){return t.avhapps}),d3.max(allData,function(t){return t.avhapps})]),indices=Array(allData.length-1)
for(var w=0;w<allData.length-1;w++)indices[w]=w
...
for(var w=0;w<allData.length-1;w++)sortedStates[w]=[w,indices[w],allStateNames[indices[w]]]
sortedStateList=Array(allData.length)
for(var w=0;w<allData.length;w++)sortedStateList[indices[w]]=w+1
stateFeatures=topojson.feature(r,r.objects.states).features
var b=c.selectAll("path").data(stateFeatures)
b.enter().append("path").attr("d",function(t,e){return m(t.geometry)}).attr("id",function(t,e){return t.properties.name}).attr("class",function(t,e){return"state map "+t.properties.name[0]+t.properties.name.split(" ")[t.properties.name.split(" ").length-1]+" q"+classColor(sortedStateList[e])+"-8"}).on("mousedown",n).on("mouseover",a).on("mouseout",o),b.exit().remove(),b.attr("stroke","black").attr("stroke-width",".7"),d3.select(window).on("resize.map",s)}}}(),hedotools.sankeyoncall=function(){return{test:function(t,e){console.log("set in module"),console.log(allDataOld),hedotools.shifter.shift(allDataOld[e[t].index].freq,allData[e[t].index].freq,lens,words)
var r=hedotools.shifter._compH()>hedotools.shifter._refH()?"happier":"less happy"
...
N[N.length-1]=(N[N.length-1]+1)%2,q.selectAll("text.shifttext").transition().duration(1e3).text(function(t,e){return N[e]?c[e]:i[e]})})}function n(){return 1}var o,s,i,l,c,u,f,h,d,p,x,m,g,v,y,w,b,A,k,z,F,D,C,M,R,S,T,Q,L,H,E,q,I,W,N,B,P,X,j,U,O,G,V,Y,J,K,Z,_=d3.urllib.encoder().varname("wordtypes"),$=d3.urllib.decoder().varname("wordtypes").varresult("none"),tt=!1,b=-1,et=d3.urllib.encoder().varname("viz"),rt=d3.urllib.decoder().varname("viz").varresult("wordshift"),nt=!1,at=function(t){var e=this
