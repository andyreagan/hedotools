// namespace it
// hedonometer.org/maps.html needs this in hedotools.map.js
// begin with some helper functions
// http://stackoverflow.com/a/1026087/3780153
// this works really well, but it's deadly slow (working max 5 elements)
// and it's coupled to jquery
// http://stackoverflow.com/a/5047712/3780153
// yup
// http://stackoverflow.com/questions/3883342/add-commas-to-a-number-in-jquery
// look away
// current usage example:
// (from the sankey page)
//
// hedotools.shifter.shift(allDataOld[hedotools.sankey.newindices()[0]].freq,allDatra[hedotools.sankey.newindices()[0]].freq,lens,words);
// hedotools.shifter.setfigure(d3.select("#shift01")).setHeight(400).setText("Why "+allDataOld[hedotools.sankey.newindices()[0]].name+" has become "+"happier"+":").plot();
//
// there are two options for having it compute the shift
// calling the .shift() with four arguments does the trick
// or calling .shifter() with no arguments also does 
// for the latter, need to have defined the variables beforehand
// using the _lens, _words, etc accessors
//
// if the text isn't set, will attempt to grab it using the
// allData structure (which works in the maps.html)
//
// the text setting here removes all p instances in the figure,
// and then inserts a new one before the svg, inside the figure (using d3.insert)
//
// can also use the setText method to set the text
// define the shifter module 
// my_array = Array(10000);
// for (var i=0; i<my_array.length; i++) { my_array[i] = i; }
function capitaliseFirstLetter(t){return t.charAt(0).toUpperCase()+t.slice(1)}function commaSeparateNumber(t){for(;/(\d+)(\d{3})/.test(t.toString());)t=t.toString().replace(/(\d+)(\d{3})/,"$1,$2")
return t}function splitWidth(t,e){for(var r=t.split(" "),n=[r[0]],a=1,s=0;a<r.length;)(n[s]+r[a]).width()<e?n[s]+=" "+r[a]:(s++,n.push(r[a])),a++
return n}function cloudSpriteSimple(t,e,r){if(!e.sprite){t.setTransform(1,0,0,1,0,0),t.clearRect(0,0,r[0],r[1]),t.save()
t.font=e.fontString,t.textAlign="center",t.textBaseline="bottom",t.fillStyle="red",t.strokeStyle="red",t.textAlign="center",t.lineWidth=2*e.padding
var n=t.measureText(e.text+"m").width,a=e.size
e.x0=Math.floor(-n/2),e.x1=Math.ceil(n/2),e.y0=0,e.y1=Math.ceil(a),t.translate(r[0]/2,r[1]/2),t.rotate(e.rotate*cloudRadians),t.fillText(e.text,0,0),t.strokeText(e.text,0,0)
var s=t.getImageData(r[0]/2+e.x0,r[1]/2+e.y0-e.y1,2*e.x1,e.y1).data
e.sprite=monochrome(s),t.restore()}}function monochrome(t){for(var e=Array(t.length/4),r=0;r<e.length;r++){var n=t[4*r]
e[r]=n>25.6?1:0}return e}function collideRects(t,e){return e.x0<t.x1&&e.x1>t.x0&&e.y1>t.y0&&e.y0>t.y1}function indexer2d(t,e,r,n,a,s){for(var i=Array((n-r+1)*(s-a+1)),o=r;o<=n;o++)for(var l=a;l<=s;l++)i[(o-r)*(s-a+1)+(l-a)]=t[e*o+l]
return i}var hedotools=hedotools||{},classColor=d3.scaleQuantize().range([0,1,2,3,4,5,6]).domain([50,1])
String.prototype.width=function(t){var e=t||"12px arial",r=$("<div>"+this+"</div>").css({position:"absolute",float:"left","white-space":"nowrap",visibility:"hidden",font:e}).appendTo($("body")),n=r.width()
return r.remove(),n},Array.prototype.findIndexClosest=function(t){for(var e=0,r=Math.abs(t-this[0]),n=1;n<this.length;n++)r>Math.abs(t-this[n])&&(e=n,r=Math.abs(t-this[n]))
return e},String.prototype.safe=function(){var t=this.split("/")
return t[t.length-1]=escape(t[t.length-1]),t.join("/")}
var intStr0=["zero","one","two","three","four","five","six","seven","eight","nine","then"],intStr=intStr0.slice(1,100)
hedotools.shifter=function(){function t(){it.selectAll(".sumrectR,.sumrectL").styles({opacity:"0.7"}),tt=!1,w=-1,it.selectAll("rect.shiftrect").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?zt:k(t))+","+R(e+1)+")"}),it.selectAll("text.shifttext").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?k(t)+2:k(t)-2)+","+(R(e+1)+wt)+")"}),_.varval("none"),_.destroy()}function e(e){if(it.selectAll(".resetbutton").remove(),e){var r=C.append("g").attr("transform","translate(4,"+(56+q)+") rotate(-90)").attr("class","resetbutton")
r.append("rect").attr("x",0).attr("y",0).attr("rx",3).attr("ry",3).attr("width",48).attr("height",17).attr("fill","#F0F0F0").styles({"stroke-width":"0.5",stroke:"rgb(0,0,0)"}),r.append("text").text("Reset").attr("x",6).attr("y",13).attr("font-size",De),r.append("rect").attr("x",0).attr("y",0).attr("rx",3).attr("ry",3).attr("width",48).attr("height",18).attr("fill","white").styles({opacity:"0.0"}).on("click",function(){t()})}}function r(){console.log("adding the translate button")
var t=C.append("g").attr("class","translatebutton").attr("transform","translate(4,"+(136+q)+") rotate(-90)")
t.append("rect").attr("x",0).attr("y",0).attr("rx",3).attr("ry",3).attr("width",75).attr("height",17).attr("fill","#F0F0F0").styles({"stroke-width":"0.5",stroke:"rgb(0,0,0)"}),t.append("text").text("Translate All").attr("x",6).attr("y",13).attr("font-size","11.0px"),t.append("rect").attr("x",0).attr("y",0).attr("rx",3).attr("ry",3).attr("width",75).attr("height",18).attr("fill","white").styles({opacity:"0.0"}).on("click",function(){for(var t=0;t<X.length-1;t++)X[t]=X[X.length-1]
X[X.length-1]=(X[X.length-1]+1)%2,T.selectAll("text.shifttext").transition().duration(1e3).text(function(t,e){return X[e]?c[e]:o[e]})})}function n(){return 1}var s,i,o,l,c,u,f,h,d,x,p,m,g,v,y,A,w,b,F,z,C,M,k,R,Q,L,D,I,N,S,H,T,P,B,X,W,E,j,q,O,V,Y,U,G,J,K,Z,_=d3.urllib.encoder().varname("wordtypes"),$=d3.urllib.decoder().varname("wordtypes").varresult("none"),tt=!1,w=-1,et=d3.urllib.encoder().varname("viz"),rt=d3.urllib.decoder().varname("viz").varresult("wordshift"),nt=!1,at=function(t){var e=this
return arguments.length?(nt=t,e):nt},st=4,it=d3.select("body"),ot=!1,lt=function(){return it},ct=function(t){var e=this
return console.log("setting figure for wordshift"),it=t,it=it.append("div").attrs({class:"outwrapper"}).styles({position:"relative"}),ot||Ct(),e},ut=!1,ft=function(t){var e=this
return arguments.length?(ut=t,vt.bottom=vt.bottom+10,e):ut},ht=function(t,e,r){for(var n,a=r||Gt[Gt.length-1]+"px  "+Fe,s=[],i=0;i<t.length;i++){if(t[i].width(a)<e)n=[t[i]]
else for(var o=t[i].split(" "),l=!1,c=0;!l;){c+=1
var u=o.slice(0,o.length-c).join(" ")
u.width(a)<e&&(n=[u,o.slice(o.length-c,o.length).join(" ")],l=!0)}s=s.concat(n)}return s},dt=550,xt=650,pt={top:0,right:0,bottom:0,left:0},mt=dt-pt.left-pt.right,gt=xt-pt.top-pt.bottom,vt={top:0,right:3,bottom:25,left:23},yt=mt-vt.left-vt.right,At=gt-vt.top-vt.bottom,wt=(vt.left,11),bt=37,Ft=64,zt=yt/2,Ct=function(){dt=d3.min([parseInt(it.style("width")),dt]),mt=dt-pt.left-pt.right,yt=mt-vt.left-vt.right,zt=yt/2},Mt=function(t){if(!arguments.length)return dt
ot=!0,dt=t,mt=dt-pt.left-pt.right,yt=mt-vt.left-vt.right,zt=yt/2},kt=function(t){var e=this
return arguments.length?(xt=t,gt=xt-pt.top-pt.bottom,At=gt-vt.top-vt.bottom,e):xt},Rt=function(t){var e=this
return arguments.length?(s=t,e):s},Qt=function(t){var e=this
return arguments.length?(i=t,e):i},Lt=function(t){var e=this
return arguments.length?(o=t,e):o},Dt=function(t){var e=this
return arguments.length?(l=t,e):l},It="Per word average happiness shift",Nt=function(t){var e=this
return arguments.length?(It=t,e):It},St="Word Rank",Ht=function(t){var e=this
return arguments.length?(St=t,e):St},Tt=function(t){var e=this
return arguments.length?(f=t,e):f},Pt=function(t){var e=this
return arguments.length?(h=t,e):h},Bt=!0,Xt=function(t){var e=this
return arguments.length?(Bt=t,e):Bt},Wt=function(t){for(var e=-1,r=0;r<g.length;r++)if(t===g[r]){e=r
break}return e},Et=function(){var t=this
return Bt=!Bt,e(Bt),Bt?(it.select("g.help").style("visibility","visible"),it.selectAll("text.credit").style("visibility","visible")):(it.select("g.help").style("visibility","hidden"),it.selectAll("text.credit").style("visibility","hidden")),t},jt=function(t,e,r,n,a,c){var d=this
return s=t,i=e,o=r,l=r,u=n,f=a,h=c,st=0,d},qt=!0,Ot=function(t){var e=this
return arguments.length?(qt=t,e):qt},Vt=1,Yt=function(t){var e=this
return arguments.length?(Vt=t,e):Vt},Ut=["#202020","#D8D8D8","#D8D8D8","#D8D8D8","#D8D8D8"],Gt=[16,12,12,12,12],Jt=function(t){var e=this
return arguments.length?(Ut=t,e):Ut},Kt=function(t){var e=this
return arguments.length?(Gt=t,e):Gt},Zt=[""],_t=function(t){var e=this
return arguments.length?(Zt=t,e):t},$t=200,te=[4,6],ee=!1,re=function(t){var e=this
return arguments.length?(te=t,e):te},ne=function(t){var e=this
return arguments.length?(d=t,$t=d3.min([d.length,$t]),st--,e):d},ae=function(t){var e=this
return arguments.length?(x=t,st--,e):x},se=function(t){var e=this
return arguments.length?(p=t,st--,e):p},ie=function(t){var e=this
return arguments.length?(m=t,e):m},oe=function(t){var e=this
return arguments.length?(g=t,st--,e):g},le=function(t){var e=this
return arguments.length?(v=t,ee=!0,e):v},ce=["nigga","niggas","niggaz","nigger"],ue=function(t){var e=this
return arguments.length?(ce=["nigga","niggas","niggaz","nigger"],ce=ce.concat(t),e):ce},fe=function(){for(var t=this,e=0;e<p.length;e++){for(var r=!0,n=0;n<ce.length;n++)ce[n]==g[e]&&(r=!1)
p[e]>te[0]&&p[e]<te[1]&&(r=!1),r||(d[e]=0,x[e]=0)}return t},he=function(t){for(var e=0;e<p.length;e++){for(var r=!0,n=0;n<ce.length;n++)ce[n]==g[e]&&(r=!1)
p[e]>te[0]&&p[e]<te[1]&&(r=!1),r||(t[e]=0)}return t},de=function(){o=o.map(function(t,e){return 0==i[e]?(e+1+". ").concat(t.concat("-↓")):1==i[e]?(e+1+". ").concat(t.concat("+↓")):2==i[e]?(e+1+". ").concat(t.concat("-↑")):(e+1+". ").concat(t.concat("+↑"))}),ee&&(c=c.map(function(t,e){return 0==i[e]?(e+1+". ").concat(t.concat("-↓")):1==i[e]?(e+1+". ").concat(t.concat("+↓")):2==i[e]?(e+1+". ").concat(t.concat("-↑")):(e+1+". ").concat(t.concat("+↑"))}))},xe=function(t,e,r,n){var a=this
return d=t,x=e,p=r,g=n,st=0,Ae(),a},pe=!1,me=function(t){var e=this
return arguments.length?(pe=t,e):pe},ge=function(){for(var t=this,e=0,r=d3.min([d.length,g.length,p.length]),n=0;n<r;n++)e+=parseFloat(d[n])
f=0
for(var n=0;n<r;n++)f+=d[n]*parseFloat(p[n])
f/=e,h=0,A=Array(r),w=Array(r)
for(var a=0,n=0;n<r;n++)a=d[n]/e,A[n]=(parseFloat(p[n])-f)*a,w[n]=a>0?2:0,parseFloat(p[n])>f&&(w[n]+=1)
for(var l=Array(r),n=0;n<r;n++)l[n]=n
l.sort(function(t,e){return Math.abs(A[t])<Math.abs(A[e])?1:Math.abs(A[t])>Math.abs(A[e])?-1:0}),s=Array($t),i=Array($t),o=Array($t)
for(var n=0;n<$t;n++)s[n]=A[l[n]],i[n]=w[l[n]],o[n]=g[l[n]]
if(pe){sortedMagFull=Array(r),y=Array(r)
for(var n=0;n<r;n++)sortedMagFull[n]=A[l[n]],y[n]=w[l[n]]}u=[0,0,0,0]
for(var n=0;n<r;n++)u[w[n]]+=A[n]
if(ee){c=Array($t)
for(var n=0;n<$t;n++)c[n]=v[l[n]]}return t},ve=function(t){var e=this
return arguments.length?(A=t,e):A},ye=function(t){var e=this
return arguments.length?(w=t,e):w},Ae=function(){for(var t=this,e=0,r=0,n=d3.min([d.length,x.length,g.length,p.length]),a=0;a<n;a++)e+=parseFloat(d[a]),r+=parseFloat(x[a])
f=0
for(var a=0;a<n;a++)f+=d[a]*parseFloat(p[a])
f/=e,h=0
for(var a=0;a<n;a++)h+=x[a]*parseFloat(p[a])
h/=r,A=Array(n),w=Array(n)
for(var m=0,a=0;a<n;a++)m=x[a]/r-d[a]/e,A[a]=(parseFloat(p[a])-f)*m,w[a]=m>0?2:0,parseFloat(p[a])>f&&(w[a]+=1)
for(var b=Array(n),a=0;a<n;a++)b[a]=a
b.sort(function(t,e){return Math.abs(A[t])<Math.abs(A[e])?1:Math.abs(A[t])>Math.abs(A[e])?-1:0}),s=Array($t),i=Array($t),o=Array($t)
for(var a=0;a<$t;a++){s[a]=A[b[a]],i[a]=w[b[a]]
var F=g[b[a]]
if(F.length>22){var z=F.slice(0,20)
" "===z[z.length-1]?o[a]=z.slice(0,z.length-1)+"…":o[a]=z+"…"}else o[a]=F}if(pe){sortedMagFull=Array(n),y=Array(n)
for(var a=0;a<n;a++)sortedMagFull[a]=A[b[a]],y[a]=w[b[a]]}u=[0,0,0,0]
for(var a=0;a<n;a++)u[w[a]]+=A[a]
if(ee){c=Array($t)
for(var a=0;a<$t;a++)c[a]=v[b[a]]}return l=o,sortedWordsRawEn=c,de(),t},we=function(){for(var t=this,e=0,r=0,n=d3.min([d.length,x.length,g.length,p.length]),a=0;a<n;a++)e+=parseFloat(d[a]),r+=parseFloat(x[a])
f=0
for(var a=0;a<n;a++)f+=d[a]*parseFloat(p[a])
f/=e,h=0
for(var a=0;a<n;a++)h+=x[a]*parseFloat(m[a])
h/=r,A=Array(n),w=Array(n)
for(var a=0;a<n;a++)A[a]=parseFloat(m[a])*x[a]/r-parseFloat(p[a])*d[a]/e,x[a]/r>d[a]/e?w[a]=2:w[a]=0,d[a]>0&&0==x[a]?parseFloat(p[a])>f&&(w[a]+=1):0==d[a]&&x[a]>0?parseFloat(m[a])>h&&(w[a]+=1):parseFloat(m[a])>parseFloat(p[a])&&(w[a]+=1)
for(var l=Array(n),a=0;a<n;a++)l[a]=a
l.sort(function(t,e){return Math.abs(A[t])<Math.abs(A[e])?1:Math.abs(A[t])>Math.abs(A[e])?-1:0}),s=Array($t),i=Array($t),o=Array($t)
for(var a=0;a<$t;a++){s[a]=A[l[a]],i[a]=w[l[a]]
var c=g[l[a]]
if(c.length>22){var v=c.slice(0,20)
" "===v[v.length-1]?o[a]=v.slice(0,v.length-1)+"…":o[a]=v+"…"}else o[a]=c}if(pe){sortedMagFull=Array(n),y=Array(n)
for(var a=0;a<n;a++)sortedMagFull[a]=A[l[a]],y[a]=w[l[a]]}u=[0,0,0,0]
for(var a=0;a<n;a++)u[w[a]]+=A[a]
return t},be=function(){var t=this,e=1
for(z=-1;e>Math.pow(10,-6);)z++,e=Math.abs(sortedMagFull[z])
b=Array(100),F=Array(100)
for(var r=Math.floor(z/100),n=0;n<100;n++){b[n]=Array(5),F[n]=Array(5)
for(var a=0;a<5;a++)b[n][a]=0,F[n][a]=0
b[n][4]=d3.sum(sortedMagFull.slice(n*r,(n+1)*r))
for(var a=n*r;a<(n+1)*r;a++)b[n][y[a]]+=sortedMagFull[a]}for(var a=0;a<5;a++)F[0][a]=b[0][a]
for(var n=1;n<100;n++)for(var a=0;a<5;a++)F[n][a]=F[n-1][a]+b[n][a]
return t},Fe="Latex default, serif",ze=function(t){var e=this
return arguments.length?(Fe=t,e):Fe},Ce=12,Me=12,ke=16,Re=12,Qe=8,Le=8,De=12,Ie=function(t){var e=this
return arguments.length?(Ce=t[0],Me=t[1],ke=t[2],Re=t[3],Qe=t[4],Le=t[5],De=t[6],e):[Ce,Me,ke,Re,Qe,Le,De]},Ne=["negdown","posdown","negup","posup"],Se=!1,He=0,Te=function(){Se=!0
var t=d3.min([q-10,80])
He=t+40,C.append("image").attrs({x:mt-t-10,y:"0",width:t,height:t,"xlink:href":"data:image/png;base64,"}).on("click",function(){window.open("http://hedonometer.org","_blank")})},Pe="shiftsvg",Be=function(t){var e=this
return arguments.length?(Pe=t,e):Pe},Xe=function(t){var e=i[t],r=1==e||3==e?"+":"-",n=1==e||3==e?"more":"less",a=e>1?"↑":"↓",c=s[t]>0?"up":"down",u=s[t]>0?"rise":"fall",m=s[t]>0?"right":"left",g=my_shifter.get_word_index(l[t])
return'The word "'+l[t]+'" is shown as "'+o[t]+"\". Let's break that down: <br><ul><li>The "+r+" means "+l[t]+" is a "+n+" happy word than the reference average ($h_{\\textrm{avg}} {(\\textrm{"+l[t]+"})}="+p[g].toFixed(2)+"$ and $h^{\\textrm{(ref)}}_{\\textrm{avg}}="+f.toFixed(2)+"$).</li><li>The "+(e>1?"↑ means used more":"↓ means used less")+"  frequently in the comparison text than reference text ("+commaSeparateNumber(x[g])+" times to "+commaSeparateNumber(d[g])+" times, such that $p_\\textrm{"+l[t]+"}^{\\textrm{(comp)}} = "+(x[g]/d3.sum(x)).toFixed(3)+"$ and $p_\\textrm{"+l[t]+"}^{\\textrm{(ref)}}="+(d[g]/d3.sum(d)).toFixed(3)+"$).</li><li>Together this brings the comparison happiness <strong>"+(s[t]>0?"up":"down")+"</strong> by "+Math.abs(s[t]).toFixed(3)+", or "+Math.abs(s[t]/(f-h)).toFixed(2)+"%. We can see this as the individual term in the shift equations' sum:   $$\\underbrace{  \\left[  h_{\\textrm{avg}} {(\\textrm{"+l[t]+"})} – h^{\\textrm{(ref)}}_{\\textrm{avg}}  \\right]  }_{"+r+"}  \\underbrace{  \\left[  p_\\textrm{"+l[t]+"}^{\\textrm{(comp)}} – p_\\textrm{"+l[t]+"}^{\\textrm{(ref)}}  \\right]    }_{\\"+c+"arrow}.$$</li></ul>"+(1==e||3==e?"Positive, ":"Negative, ")+(e>1?" more frequent (":" less frequent (")+r+a+") words  will always be on the "+m+" contributing to a "+u+" in happiness."},We=function(){console.log("launching the tour")
var t=new Tour({steps:[{element:"#shiftsvg",title:"A quick wordshift whats-what",placement:"auto left",onShown:function(){console.log("first element shown"),MathJax.Hub.Queue(["Typeset",MathJax.Hub])},content:"Let’s say we have two texts which we call ‘reference’, ${T}^\\textrm{(ref)}$, and ‘comparison’, ${T}^\\textrm{(comp)}$. We want to know why the happiness of the comparison text, $h^{\\textrm{(comp)}}_{\\textrm{avg}}$, is higher or lower than that of the reference text $h^{\\textrm{(ref)}}_{\\textrm{avg}}$.<br>To do this, we analyze how each word $w$ which contributes to the difference in average happiness between the two texts:  $$h^{\\textrm{(comp)}}_{\\textrm{avg}} – h^{\\textrm{(ref)}}_{\\textrm{avg}} =   \\sum_{w \\in L}  \\underbrace{  \\left[  h_{\\textrm{avg}} {(w)} – h^{\\textrm{(ref)}}_{\\textrm{avg}}  \\right]  }_{+/-}  \\underbrace{  \\left[  p_w^{\\textrm{(comp)}} – p_w^{\\textrm{(ref)}}  \\right]    }_{\\uparrow/\\downarrow}.$$Next let's take a look at how the top three words contribute."},{element:"#shiftrect0",content:Xe(0),placement:"auto left",onShown:function(){console.log("first element shown"),MathJax.Hub.Queue(["Typeset",MathJax.Hub])}},{element:"#shiftrect1",content:Xe(1),placement:"auto left",onShown:function(){console.log("first element shown"),MathJax.Hub.Queue(["Typeset",MathJax.Hub])}},{element:"#shiftrect2",content:Xe(2),placement:"auto left",onShown:function(){console.log("first element shown"),MathJax.Hub.Queue(["Typeset",MathJax.Hub])}},{element:"#sumTextR0",content:"At the top, we see the relative contributions from each of the four words types, and the total (the grey bar). This particular bar, $\\sum +\\uparrow$, shows the relative contribution to the shift in happiness from positive words that increased in frequency. <br><br>Try clicking this bar to see only those word types. To reset the view, click the grey $\\sum$ bar.<br><br>And that's all for now, we hope this quick what's-what was helpful!",placement:"auto left",onShown:function(){MathJax.Hub.Queue(["Typeset",MathJax.Hub])}}]})
t.init(),t.restart(),t.start(!0)},Ee=function(){it.append("a").attrs({class:"btn btn-large btn-default"}).styles({position:"absolute",left:function(){return dt-34+"px"}}).on("click",function(){We()}).append("i").attrs({class:"fa fa-question"})},je=function(){function a(){if(!(this.__zoom<=0&&d3.event.deltaY<0)){d3.event.preventDefault(),this.__zoom+=d3.event.deltaY/2
var t=this
if(T.selectAll("rect.shiftrect").attr("y",function(e){return-t.__zoom}),T.selectAll("text.shifttext").attr("y",function(e){return-t.__zoom}),pe){var e=d3.event.translate.y/(At-Ft),r=e*g*bt/p.length
it.select(".distwin").attrs({y:d3.max([2,2-r])})}}}var l=this
if(Zt[0].length<1){if(h>=f)var d="happier"
else var d="less happy"
Zt=ht(["Reference happiness: "+f.toFixed(2),"Comparison happiness: "+h.toFixed(2),"Why comparison is "+d+" than reference:"],mt-10-He)}else qt&&(Zt=ht(Zt,mt-10-He))
it.selectAll("svg").remove(),C=it.append("svg").attr("id",Pe).attr("width",function(){return mt}).attr("height",function(){return gt})
bgbgrect=C.append("rect").attr("x",0).attr("y",0).attr("width",mt).attr("height",gt).attr("class","bgbg").attr("fill","rgba(255,248,220,.2)"),q=17*Zt.length+13,At=gt-vt.top-vt.bottom-q,M=d3.max(o.slice(0,7).map(function(t){return t.width(Re+"px  "+Fe)}))
if(k=d3.scaleLinear().domain([-Math.abs(s[0]),Math.abs(s[0])]).range([M+10,yt-M-10]),R=d3.scaleLinear().domain([bt+1,1]).range([At+2,Ft]),H=d3.zoom().translateExtent([[Number.NEGATIVE_INFINITY,Number.NEGATIVE_INFINITY],[Number.POSITIVE_INFINITY,Number.POSITIVE_INFINITY]]).on("zoom",a),T=C.append("g").attr("transform","translate("+vt.left+","+(vt.top+q)+")").attr("width",yt).attr("height",At).attr("class","main"),T.on("wheel.zoom",a),L=T.append("rect").attrs({x:0,y:1,width:yt-2,height:At-2,class:"bg"}).styles({"stroke-width":"0.5",stroke:"rgb(0,0,0)",fill:"#FCFCFC",opacity:"0.96"}),ut){V=function(){return d3.axisBottom().ticks(4).scale(k)}().innerTickSize(3).outerTickSize(0),C.append("g").attr("class","x axis ").attr("font-size",Me).attr("transform","translate("+vt.left+","+(Ft+At)+")").call(V),C.selectAll(".tick line").styles({stroke:"black"})}Ne=["negdown","posdown","negup","posup"]
var x=["#b3b3ff","#ffffb3","#4c4cff","#ffff4c","#272727"]
if(P=T.selectAll("rect.shiftrect").data(s).enter().append("rect").attrs({class:function(t,e){return"shiftrect "+intStr0[i[e]]+" "+Ne[i[e]]},id:function(t,e){return"shiftrect"+e},x:0,y:0,transform:function(t,e){return t>0?"translate("+zt+","+R(e+1)+")":"translate("+k(t)+","+R(e+1)+")"},height:function(t,e){return wt},width:function(t,e){return t>0?k(t)-k(0):k(0)-k(t)},opacity:"0.7","stroke-width":"1",stroke:"rgb(0,0,0)",fill:function(t,e){return x[i[e]]}}),B=T.selectAll("text.shifttext").data(s).enter().append("text").attrs({class:function(t,e){return"shifttext "+intStr0[i[e]]},x:0,y:0,transform:function(t,e){return t>0?"translate("+(k(t)+2)+","+(R(e+1)+wt)+")":"translate("+(k(t)-2)+","+(R(e+1)+wt)+")"}}).styles({"text-anchor":function(t,e){return t<0?"end":"start"},"font-size":Re}).text(function(t,e){return o[e]}),ee){X=Array(o.length+1)
for(var m=0;m<X.length;m++)X[m]=0
X[X.length-1]=1,B.on("click",function(t,e){X[e]?(d3.select(this).text(o[e]),X[e]=0):(d3.select(this).text(c[e]),X[e]=1)})}if("posup"===$().current?(tt=!0,w=3,e(!0),T.selectAll("rect.shiftrect.zero").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.zero").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.one").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.one").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.two").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.two").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.three").attr("transform",function(t,e){return"translate("+(t>0?zt:k(t))+","+R(e+1)+")"}),T.selectAll("text.shifttext.three").attr("transform",function(t,e){return"translate("+(t>0?k(t)+2:k(t)-2)+","+(R(e+1)+wt)+")"})):"negdown"===$().current?(tt=!0,w=0,e(!0),T.selectAll("rect.shiftrect.zero").attr("transform",function(t,e){return"translate("+(t>0?zt:k(t))+","+R(e+1)+")"}),T.selectAll("text.shifttext.zero").attr("transform",function(t,e){return"translate("+(t>0?k(t)+2:k(t)-2)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.one").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.one").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.two").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.two").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.three").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.three").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"})):"posdown"===$().current?(tt=!0,w=1,e(!0),T.selectAll("rect.shiftrect.zero").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.zero").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.three").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.three").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.two").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.two").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.one").attr("transform",function(t,e){return"translate("+(t>0?zt:k(t))+","+R(e+1)+")"}),T.selectAll("text.shifttext.one").attr("transform",function(t,e){return"translate("+(t>0?k(t)+2:k(t)-2)+","+(R(e+1)+wt)+")"})):"negup"===$().current&&(tt=!0,w=2,e(!0),T.selectAll("rect.shiftrect.zero").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.zero").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.one").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.one").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.two").attr("transform",function(t,e){return"translate("+(t>0?zt:k(t))+","+R(e+1)+")"}),T.selectAll("text.shifttext.two").attr("transform",function(t,e){return"translate("+(t>0?k(t)+2:k(t)-2)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.three").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.three").attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"})),I=T.append("rect").attr("x",3).attr("y",3).attr("width",yt-vt.left-5).attr("height",60).attr("fill","white").styles({opacity:"1.0"}),topbgrect2=C.append("rect").attr("x",0).attr("y",0).attr("width",mt).attr("height",q).attr("fill","white").styles({opacity:"1.0"}),j=C.selectAll("text.titletext").data(Zt).enter().append("text").attr("y",function(t,e){return 17*(e+1)}).attr("x",3).attr("class",function(t,e){return"titletext "+intStr[e]}).styles({"font-size":function(t,e){return Gt[e]},"line-height":"1.42857143",color:function(t,e){return Ut[e]},"font-weight":function(t,e){return e<Vt?"bold":"normal"}}).text(function(t,e){return t}),bottombgrect=T.append("rect").attr("x",3).attr("y",xt-vt.bottom-q).attr("width",yt-2).attr("height",vt.bottom).attr("fill","white").styles({opacity:"1.0"}),S=T.append("line").attrs({x1:0,x2:yt-2,y1:60,y2:60}).styles({"stroke-width":"1",stroke:"black"}),W=Math.max(Math.abs(u[1]),Math.abs(u[2]),u[0],u[3],d3.sum(u)),Q=d3.scaleLinear().domain([-W,W]).range([34.1562,yt-34.1562]),E=[u[3],u[0],d3.sum(u)],Ne=["posup","negdown","sumgrey"],x=["#ffff4c","#b3b3ff","#272727"],T.selectAll(".sumrectR").data(E).enter().append("rect").attrs({class:function(t,e){return"sumrectR "+intStr0[e]+" "+Ne[e]},x:function(t,e){return t>0?zt:Q(t)},y:function(t,e){return e<3?17*e+7:17*e+7-2},height:function(t,e){return 14},width:function(t,e){return t>0?Q(t)-zt:zt-Q(t)},fill:function(t,e){return x[e]}}).styles({opacity:function(t,e){var r=[3,0,-1]
return tt&&w!==r[e]?"0.14":"0.7"},"stroke-width":"1",stroke:"rgb(0,0,0)"}).on("mouseover",function(t,e){var r=[3,0,-1]
if(tt)if(w===r[e]){d3.select(this).styles({opacity:"0.7"})}else{d3.select(this).styles({opacity:"0.3"})}else{d3.select(this).styles({opacity:"1.0"})}}).on("mouseout",function(t,e){var r=[3,0,-1]
if(tt)if(w===r[e]){d3.select(this).styles({opacity:"0.7"})}else{d3.select(this).styles({opacity:"0.14"})}else{d3.select(this).styles({opacity:"0.7"})}}).on("click",function(r,n){var a=[3,0,-1]
it.selectAll(".sumrectR,.sumrectL").styles({opacity:"0.1"})
d3.select(this).styles({opacity:"0.7"})
0==n?(tt=!0,w=a[n],e(!0),_.varval("posup"),T.selectAll("rect.shiftrect.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?zt:k(t))+","+R(e+1)+")"}),T.selectAll("text.shifttext.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?k(t)+2:k(t)-2)+","+(R(e+1)+wt)+")"})):1==n?(tt=!0,w=a[n],e(!0),_.varval("negdown"),T.selectAll("rect.shiftrect.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?zt:k(t))+","+R(e+1)+")"}),T.selectAll("text.shifttext.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?k(t)+2:k(t)-2)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"})):2==n&&t()}),T.selectAll(".sumtextR").data([u[3],u[0],d3.sum(u)]).enter().append("text").styles({"text-anchor":function(t,e){return t>0?"start":"end"},"font-size":Ce}).attrs({class:"sumtextR",id:function(t,e){return"sumTextR"+e},y:function(t,e){return 17*e+17},x:function(t,e){return Q(t)+5*t/Math.abs(t)}}).text(function(t,e){return 0==e?"∑+↑":1==e?"∑-↓":"∑"}),E=[u[1],u[2]],Ne=["posdown","negup"],x=["#ffffb3","#4c4cff"],T.selectAll(".sumrectL").data(E).enter().append("rect").attrs({class:function(t,e){return"sumrectL "+intStr0[e]+" "+Ne[e]},id:function(t,e){return"sumTextL"+e},x:function(t,e){return e<2?Q(t):(u[3]+u[1])*(u[0]+u[2])>0?t>0?Q(u[3]+u[1]):Q(t)-(zt-Q(u[3]+u[1])):t>0?zt:Q(t)},y:function(t,e){return 17*e+7},height:function(t,e){return 14},width:function(t,e){return t>0?Q(t)-zt:zt-Q(t)},fill:function(t,e){return x[e]}}).styles({opacity:function(t,e){var r=[1,2]
return tt&&w!==r[e]?"0.14":"0.7"},"stroke-width":"1",stroke:"rgb(0,0,0)"}).on("mouseover",function(t,e){var r=[1,2]
if(tt)if(w===r[e]){d3.select(this).styles({opacity:"0.7"})}else{d3.select(this).styles({opacity:"0.3"})}else{d3.select(this).styles({opacity:"1.0"})}}).on("mouseout",function(t,e){var r=[1,2]
if(tt)if(w===r[e]){d3.select(this).styles({opacity:"0.7"})}else{d3.select(this).styles({opacity:"0.14"})}else{d3.select(this).styles({opacity:"0.7"})}}).on("click",function(t,r){var n=[1,2]
tt=!0,w=n[r],it.selectAll(".sumrectR,.sumrectL").styles({opacity:"0.1"})
d3.select(this).styles({opacity:"0.7"})
e(!0),0==r?(_.varval("posdown"),T.selectAll("rect.shiftrect.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?zt:k(t))+","+R(e+1)+")"}),T.selectAll("text.shifttext.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?k(t)+2:k(t)-2)+","+(R(e+1)+wt)+")"})):1==r&&(_.varval("negup"),T.selectAll("rect.shiftrect.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?zt:k(t))+","+R(e+1)+")"}),T.selectAll("text.shifttext.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?k(t)+2:k(t)-2)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}))}),T.selectAll(".sumtextL").data(E).enter().append("text").styles({"text-anchor":"end","font-size":Ce}).attr("class","sumtextL").attr("y",function(t,e){return 17*e+17}).text(function(t,e){return 0==e?"∑+↓":"∑-↑"}).attr("x",function(t,e){return Q(t)-5}),D=C.append("text").text(It).attr("class","axes-text").attr("x",vt.left+zt).attr("y",gt-7).styles({"font-size":ke,fill:"#000000","text-anchor":"middle"}),N=C.append("text").text(St).attr("class","axes-text").attr("x",18).attr("y",At/2+60+q).attr("font-size",ke).attr("fill","#000000").attr("transform","rotate(-90.0,18,"+(At/2+60+q)+")"),T.property("__zoom",0),pe){be()
var g=250
Y=T.append("g").attrs({class:"dist",fill:"#B8B8B8",transform:"translate(5,"+(At-28-g)+")"}),Y.append("rect").attrs({x:0,y:0,height:g,width:70,class:"distbg","stroke-width":"2",stroke:"rgb(150,150,150)",fill:"#FCFCFC",opacity:"0.96"})
var v=d3.scaleLinear().domain(d3.extent(b.map(function(t){return t[4]}))).range([1,68]),y=d3.scaleLinear().domain([0,99]).range([2,g-2]),A=d3.line().x(function(t,e){return v(t)}).y(function(t,e){return y(e)}).curve(d3.curveCardinal),U=(Y.append("path").datum(b.map(function(t){return t[4]})).attr("class","line").attr("d",A).attr("stroke","red").attr("stroke-width",1.25).attr("fill","none"),d3.scaleLinear().domain(d3.extent(F.map(function(t){return t[4]}))).range([1,68])),G=d3.line().x(function(t,e){return U(t)}).y(function(t,e){return y(e)}).curve(d3.curveCardinal)
Y.append("path").datum(F.map(function(t){return t[4]})).attr("class","line").attr("d",G).attr("stroke","blue").attr("stroke-width",1.25).attr("fill","none"),Y.append("rect").attrs({x:0,y:2,height:g*bt/z,width:70,class:"distwin","stroke-width":"0.75",stroke:"rgb(20,20,20)",fill:"#FCFCFC",opacity:"0.6"}),Y.append("text").attrs({x:72,y:g+2,class:"nwordslabel"}).styles({fill:"#B8B8B8","font-size":Qe,"text-anchor":"start"}).text(z)
Y.append("text").attrs({x:72,y:2,class:"zerolabel"}).styles({fill:"#B8B8B8","font-size":Qe,"text-anchor":"start"}).text("0")}return O=T.selectAll("text.credit").data(["visualization by","@andyreagan"]).enter().append("text").attrs({class:"credit",x:yt-5,y:function(t,e){return At-15+10*e}}).styles({"text-anchor":"end",fill:"#B8B8B8","font-size":Le}).text(function(t){return t}),ot||d3.select(window).on("resize.shiftplot",n),Bt&&e(!0),ee&&(console.log(ee),r()),l},qe=function(){var t=this
"table"===rt().cached&&(U.remove(),G.remove(),J.remove(),K.remove(),Z.remove()),et.varval("cloud"),T.selectAll("rect.shiftrect").transition().duration(1e3).attr("x",1e3),T.selectAll(".sumrectR").transition().duration(2e3).attr("x",1e3),T.selectAll(".sumtextR").transition().duration(2e3).attr("x",1e3),T.selectAll(".sumrectL").transition().duration(2e3).attr("x",-1e3),T.selectAll(".sumtextL").transition().duration(2e3).attr("x",-1e3),bottombgrect.attr("y",1e3),D.transition().duration(2e3).attr("y",1e3),N.transition().duration(2e3).attr("x",-1e3),I.transition().duration(2e3).attr("y",-200),S.transition().duration(1e3).attrs({y1:-10,y2:-10})
var e=d3.scaleLinear().domain([Math.abs(s[49]),Math.abs(s[0])]).range([6,70])
it.selectAll("canvas").remove(),it.append("canvas").attrs({width:yt,height:At,id:"cloudcanvas"}).styles({display:"none"})
var r=document.getElementById("cloudcanvas").getContext("2d")
it.append("canvas").attrs({width:yt,height:At,id:"cloudcanvas2"}).styles({display:"none"})
var n=document.getElementById("cloudcanvas2").getContext("2d")
r.textAlign="center",r.fillStyle="red",r.strokeStyle="red",r.textAlign="center",r.save()
for(var i=l.map(function(t,r){return a={},a.text=t,a.rotate=90*Math.round(1*Math.random()-.5),a.size=e(Math.abs(s[r])),a.fontString=a.size+"px  "+Fe,a.padding=1,a.x=zt+(Math.random()-.5)*yt/2,a.y=At/2,a}),o=[yt,At],c=[],u=-1;u++<199;){var f=i[u]
cloudSpriteSimple(n,f,o)
var h=monochrome(r.getImageData(0,0,o[0],o[1]).data);(function(t,e,r){var n=(o[0],o[1],e.x),a=e.y,s=Math.sqrt(o[0]*o[0]+o[1]*o[1]),i=o[0]/o[1],l=Math.random()<.5?1:-1
l*=2
for(var c,u,f,h=-l;(c=function(t){return[i*t*Math.cos(t),t*Math.sin(t)]}(h+=l))&&(u=c[0],f=c[1],!(Math.min(Math.abs(u),Math.abs(f))>=s));)if(e.x=n+u,e.y=a+f,!(e.x+e.x0<0||e.y+e.y0<0||e.x+e.x1>o[0]||e.y+e.y1>o[1])){var d={x0:Math.floor(e.x)+e.x0,y0:Math.floor(e.y)-e.y1,w:2*e.x1,h:e.y1}
d.x1=d.x0+d.w,d.y1=d.y0+d.h
for(var x=indexer2d(r,o[0],d.y0,d.y1,d.x0,d.x1),p=0;p<e.sprite.length&&(!e.sprite[p]||!x[p]);p++);if(p===e.sprite.length)return t.push(d),!0}!1})(c,f,h)
r.setTransform(1,0,0,1,0,0),r.save(),r.font=f.fontString,r.lineWidth=2*f.padding,r.translate(f.x,f.y),r.rotate(f.rotate*cloudRadians),r.fillText(f.text,0,0),r.strokeText(f.text,0,0),r.restore()}return T.selectAll("text.shifttext").data(i).filter(function(t,e){if(e<200)return 1}).text(function(t,e){return t.text}).transition().duration(1e3).delay(function(t,e){return 10*e}).attr("placed",function(t,e){return t.placed}).attr("x",function(t,e){return 0}).attr("y",function(t,e){return 0}).style("text-anchor","middle").style("font",function(t,e){return t.fontString}).attr("transform",function(t,e){return"translate("+[t.x,t.y]+")rotate("+t.rotate+")"}),t},Oe=function(){var t=this
et.varval("table")
var r=20,n=r+70+25+25+35,a=yt-n,u=["start","start","middle","middle","end","middle"]
xpadding=10
var d=n+a/2,x=[0,r,r+70,r+70+25,r+70+25+25+35,d]
k.domain([-Math.abs(s[0]),Math.abs(s[0])]).range([xpadding,a-xpadding]),Ft=15,R.range([At+2,Ft]),e(!1)
var p=T.selectAll("rect.shiftrect").data(s),m=T.selectAll("text.shifttext").data(s)
return p.transition().duration(1e3).attrs({fill:"grey",class:function(t,e){return"shiftrect "+intStr0[i[e]]},transform:function(t,e){return t>0?"translate("+d+","+(R(e+1)+2.5)+")":"translate("+(k(t)+n)+","+(R(e+1)+2.5)+")"},height:function(t,e){return wt},width:function(t,e){return t>0?k(t)-k(0):k(0)-k(t)}}),m.transition().duration(1e3).attrs({class:function(t,e){return"shifttext "+intStr0[i[e]]},transform:function(t,e){return"translate("+r+","+(R(e+1)+wt)+")"}}).styles({"text-anchor":"start","font-size":Re}).text(function(t,e){return l[e]}),o=l,ee&&(c=void 0),U=T.selectAll("text.shiftrank").data(s).enter().append("text").attr("y",function(t,e){return R(e+1)+wt}).attr("x",function(t,e){return 0}).styles({"text-anchor":"start","font-size":Re}).text(function(t,e){return e+1+"."}),J=T.selectAll("text.shifttype").data(s).enter().append("text").attr("y",function(t,e){return R(e+1)+wt}).attr("x",function(t,e){return r+70}).styles({"text-anchor":"middle","font-size":Re}).text(function(t,e){return 0==i[e]?"-":2==i[e]?"-":"+"}),G=T.selectAll("text.shiftfreq").data(s).enter().append("text").attr("y",function(t,e){return R(e+1)+wt}).attr("x",function(t,e){return r+70+25}).styles({"text-anchor":"middle","font-size":Re}).text(function(t,e){return 0==i[e]?"↓":1==i[e]?"↓":"↑"}),K=T.selectAll("text.shiftmag").data(s).enter().append("text").attr("y",function(t,e){return R(e+1)+wt}).attr("x",function(t,e){return r+70+25+25+35}).styles({"text-anchor":"end","font-size":Re}).text(function(t,e){return(t/Math.abs(h-f)*100).toFixed(2)+"%"}),Z=T.selectAll("text.header").data(["#","Word","+/-","↑/↓","% Cont.","% Cont."]).enter().append("text").attrs({x:function(t,e){return x[e]},y:function(t,e){return R(0)+wt}}).styles({"text-anchor":function(t,e){return u[e]},"font-size":function(t,e){return Re}}).text(function(t,e){return t}),T.selectAll(".sumrectR").transition().duration(2e3).attr("x",1e3),T.selectAll(".sumtextR").transition().duration(2e3).attr("x",1e3),T.selectAll(".sumrectL").transition().duration(2e3).attr("x",-1e3),T.selectAll(".sumtextL").transition().duration(2e3).attr("x",-1e3),L.style("stroke-width",0),bottombgrect.attr("y",1e3),D.transition().duration(2e3).attr("y",1e3),N.transition().duration(2e3).attr("x",-1e3),I.transition().duration(2e3).attr("y",-200),S.transition().duration(1e3).attrs({y1:"15",y2:15}),t}
return{shift:xe,ignore:ue,setTextBold:Yt,stop:fe,istopper:he,shifter:Ae,selfShifter:ge,setfigure:ct,getfigure:lt,setdata:jt,plot:je,add_help_button:Ee,show_x_axis:ft,replot:function(){var t=this
if(R.range([At+2,64]),S.transition().duration(1e3).attrs({y1:60,y2:60}),"table"===rt().cached&&(U.remove(),G.remove(),J.remove(),K.remove(),Z.remove()),et.varval("wordshift"),Zt[0].length<1){if(h>=f)var r="happier"
else var r="less happy"
Zt=ht(["Reference happiness: "+f.toFixed(2),"Comparison happiness: "+h.toFixed(2),"Why comparison is "+r+" than reference:"],mt-10-He,"14px  "+Fe)}else qt&&(Zt=ht(Zt,mt-10-He,"14px  "+Fe))
M=d3.max(o.slice(0,5).map(function(t){return t.width(Re+"px  "+Fe)}))
k.domain([-Math.abs(s[0]),Math.abs(s[0])]).range([M+10,yt-M-10]),ut&&C.select(".x.axis").call(V),q=17*Zt.length+13,e(!0),At=gt-vt.top-vt.bottom-q,R.range([At+2,64]),T.attr("transform","translate("+vt.left+","+(vt.top+q)+")").attr("height",At),L.attr("height",At-2).styles({"stroke-width":.5,stroke:"rgb(20,20,20)"}),topbgrect2.attr("height",q),C.selectAll("text.credit").remove(),O.remove(),O=T.selectAll("text.credit").data(["visualization by","@andyreagan"]).enter().append("text").attrs({class:"credit",fill:"#B8B8B8",x:yt-5,y:function(t,e){return At-15+10*e},"font-size":"8.0px"}).styles({"text-anchor":"end"}).text(function(t){return t}),C.selectAll("text.titletext").remove(),j.remove(),j=C.selectAll("text.titletext").data(Zt).enter().append("text").attrs({y:function(t,e){return 17*(e+1)},x:3,class:function(t,e){return"titletext "+intStr[e]}}).styles({"font-size":function(t,e){return Gt[e]},"line-height":"1.42857143",color:function(t,e){return Ut[e]},"font-weight":function(t,e){return e<Vt?"bold":"normal"}}).text(function(t,e){return t}),bottombgrect.attr("y",xt-vt.bottom-q)
var n=T.selectAll("rect.shiftrect").data(s),a=T.selectAll("text.shifttext").data(s)
return"none"===$().current||0===$().current.length?(n.transition().duration(1500).attrs({fill:function(t,e){return 2==i[e]?"#4C4CFF":3==i[e]?"#FFFF4C":0==i[e]?"#B3B3FF":"#FFFFB3"},class:function(t,e){return"shiftrect "+intStr0[i[e]]},x:0,y:0,transform:function(t,e){return t>0?"translate("+zt+","+R(e+1)+")":"translate("+k(t)+","+R(e+1)+")"},height:function(t,e){return wt},width:function(t,e){return t>0?k(t)-k(0):k(0)-k(t)}}),a.transition().duration(1500).attrs({class:function(t,e){return"shifttext "+intStr0[i[e]]},x:0,y:0,transform:function(t,e){return t>0?"translate("+(k(t)+2)+","+(R(e+1)+wt)+")":"translate("+(k(t)-2)+","+(R(e+1)+wt)+")"}}).styles({"text-anchor":function(t,e){return s[e]<0?"end":"start"},"font-size":Re}).text(function(t,e){return o[e]})):(n.attrs({fill:function(t,e){return 2==i[e]?"#4C4CFF":3==i[e]?"#FFFF4C":0==i[e]?"#B3B3FF":"#FFFFB3"},class:function(t,e){return"shiftrect "+intStr0[i[e]]},x:0,y:0,transform:function(t,e){return t>0?"translate("+zt+","+R(e+1)+")":"translate("+k(t)+","+R(e+1)+")"},height:function(t,e){return wt},width:function(t,e){return t>0?k(t)-k(0):k(0)-k(t)}}),a.attrs({class:function(t,e){return"shifttext "+intStr0[i[e]]},x:0,y:0,transform:function(t,e){return t>0?"translate("+(k(t)+2)+","+(R(e+1)+wt)+")":"translate("+(k(t)-2)+","+(R(e+1)+wt)+")"}}).styles({"text-anchor":function(t,e){return s[e]<0?"end":"start"},"font-size":Re}).text(function(t,e){return o[e]}),"posup"===$().current?(T.selectAll("rect.shiftrect.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?zt:k(t))+","+R(e+1)+")"}),T.selectAll("text.shifttext.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?k(t)+2:k(t)-2)+","+(R(e+1)+wt)+")"})):"negdown"===$().current?(T.selectAll("rect.shiftrect.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?zt:k(t))+","+R(e+1)+")"}),T.selectAll("text.shifttext.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?k(t)+2:k(t)-2)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"})):"posdown"===$().current?(T.selectAll("rect.shiftrect.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?zt:k(t))+","+R(e+1)+")"}),T.selectAll("text.shifttext.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?k(t)+2:k(t)-2)+","+(R(e+1)+wt)+")"})):"negup"===$().current&&(T.selectAll("rect.shiftrect.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.zero").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.one").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?zt:k(t))+","+R(e+1)+")"}),T.selectAll("text.shifttext.two").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?k(t)+2:k(t)-2)+","+(R(e+1)+wt)+")"}),T.selectAll("rect.shiftrect.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+R(e+1)+")"}),T.selectAll("text.shifttext.three").transition().duration(1e3).attr("transform",function(t,e){return"translate("+(t>0?500:-500)+","+(R(e+1)+wt)+")"}))),W=Math.max(Math.abs(u[1]),Math.abs(u[2]),u[0],u[3]),Q.domain([-W,W]),E=[u[3],u[0],d3.sum(u)],T.selectAll(".sumrectR").data(E).transition().duration(1500).attr("x",function(t,e){return t>0?zt:Q(t)}).attr("width",function(t,e){return t>0?Q(t)-zt:zt-Q(t)}),T.selectAll(".sumtextR").data([u[3],u[0],d3.sum(u)]).transition().duration(1500).attr("class","sumtextR").style("text-anchor",function(t,e){return t>0?"start":"end"}).attr("x",function(t,e){return Q(t)+5*t/Math.abs(t)}),E=[u[1],u[2],u[0]+u[2]],T.selectAll(".sumrectL").data(E).transition().duration(1500).attr("fill",function(t,e){return 0==e?"#FFFFB3":1==e?"#4C4CFF":t>0?"#B3B3FF":"#4C4CFF"}).attr("x",function(t,e){return e<2?Q(t):(u[3]+u[1])*(u[0]+u[2])>0?t>0?Q(u[3]+u[1]):Q(t)-(zt-Q(u[3]+u[1])):t>0?zt:Q(t)}).attr("width",function(t,e){return t>0?Q(t)-zt:zt-Q(t)}),T.selectAll(".sumtextL").data([u[1],u[2]]).transition().duration(1500).attr("x",function(t,e){return Q(t)-5}),t},table:Oe,cloud:qe,setText:_t,setWidth:Mt,setHeight:kt,splitstring:ht,drawlogo:Te,resetbuttontoggle:Et,plotdist:me,_reset:Xt,_stoprange:re,_refF:ne,_compF:ae,_lens:se,_complens:ie,_words:oe,_words_en:le,_refH:Tt,_compH:Pt,_xlabel_text:Nt,_ylabel_text:Ht,setTextColors:Jt,setTopTextSizes:Kt,_split_top_strings:Ot,_shiftMag:ve,_shiftType:ye,dualShifter:we,setFontSizes:Ie,_fontString:ze,_viz_type_use_URL:at,_my_shift_id:Be,_sortedMag:Rt,_sortedType:Qt,_sortedWords:Lt,_sortedWordsRaw:Dt,get_word_index:Wt}}
var cloudRadians=Math.PI/180
